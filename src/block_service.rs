...
pub struct BlockService<'db, Tx: Transaction, CF: CustomFamilies<'db>> {
    pub(crate) tx_service: &'db dyn TxService<'db, CF = CF, Tx = Tx>,
    pub(crate) families: &'db Families<'db, CF>,
...
impl<'db, Tx: Transaction, CF: CustomFamilies<'db>> BlockService<'db, Tx, CF> {
    pub fn new(
        service: &'db dyn TxService<'db, CF = CF, Tx = Tx>,
        families: &'db Families<'db, CF>,
    ) -> Self {
        BlockService {
            tx_service: service,
            families,
    ...
    pub(crate) fn persist_block(
        ...
        self.tx_service
            .persist_txs(&block, db_tx, batch, tx_pk_by_tx_hash_lru_cache)?;
        self.persist_header(&block.header, db_tx, batch)?;
    ...
    pub(crate) fn remove_block(
        ...
        self.remove_header(&block.header, db_tx, &mut block_height_by_hash_lru_cache)
    }
    ...
    pub(crate) fn update_blocks(
        ...
        let removed_blocks: Result<Vec<Rc<Block<Tx>>>, String> = blocks
            ...
            .map(|block| {
                if let Some(block_to_remove) =
                    self.get_block_by_height(block.header.height, db_tx)?
                {
                    info!("Removing block {}", block_to_remove.header);
                    self.remove_block(Rc::clone(&block_to_remove), db_tx)?;
        ...
        self.persist_blocks(blocks, db_tx, batch)
            .map_err(|e| e.into_string())?;
    ...
    fn get_block_by_hash(
        ...
        if let Some(value) = self.block_by_hash_lru_cache.borrow_mut().get(block_hash) {
            ...
            let header_opt = self.get_block_header_by_hash(block_hash, db_tx)?;
            match header_opt {
                Some(block_header) => {
                    let txs = self
                        .tx_service
                        .get_txs_by_height(&block_header.height, db_tx)?;
    ...
    fn get_block_by_height(
        ...
        let hash_bytes = db_tx.get_cf(&self.families.shared.block_hash_by_pk_cf, height_bytes)?;

        if let Some(hash) = hash_bytes.map(|bytes| codec_block::bytes_to_block_hash(&bytes)) {
            self.get_block_by_hash(&hash, db_tx)
        } else {
            Ok(None)
        }
    }
    ...
    pub(crate) fn get_block_header_by_hash(
        ...
        if let Some(value) = self.block_by_hash_lru_cache.borrow_mut().get(block_hash) {
            ...
            let header_bytes =
                db_tx.get_cf(&self.families.shared.block_pk_by_hash_cf, block_hash)?;
    ...
    pub(crate) fn persist_header(
        ...
        batch.put_cf(
            &self.families.shared.block_hash_by_pk_cf,
            height_bytes,
            block_header.hash.0,
        );
        db_tx.put_cf(
            &self.families.shared.block_pk_by_hash_cf,
            block_header.hash.0,
            header_bytes,
        )?;
    ...
    pub(crate) fn remove_header(
        ...
        db_tx
            .delete_cf(&self.families.shared.block_hash_by_pk_cf, height_bytes)

...
use crate::{db_options, info};

use super::eutxo_schema::{self, DbSchema};
...
pub fn get_db(db_schema: &DbSchema, db_path: &str) -> OptimisticTransactionDB<MultiThreaded> {
    ...
    if existing_cfs.is_empty() {
        ...
        let shared_cfs = eutxo_schema::get_shared_column_families();
        let eutxo_cfs = eutxo_schema::get_eutxo_column_families();
